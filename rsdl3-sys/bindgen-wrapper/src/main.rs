...
fn main() {
    ...
    generate_main_bindings();
}

fn generate_main_bindings() {
    let manifest_dir = PathBuf::from(std::env::var("CARGO_MANIFEST_DIR").unwrap());
    let core_include_path = manifest_dir.join("SDL").join("include");
    let core_header_path = core_include_path.join("SDL3").join("SDL_main.h");
    bindgen::Builder::default()
        .use_core()
        .clang_arg("-DSDL_MAIN_USE_CALLBACKS=1")
        .clang_arg(format!("-I{}", core_include_path.to_str().unwrap()))
        .header(core_header_path.to_str().unwrap())
        .parse_callbacks(Box::new(bindgen::CargoCallbacks::new()))
        .generate()
        .unwrap()
        .write_to_file(manifest_dir.join("../src").join("sdl_main.rs"))
        .unwrap();
}
